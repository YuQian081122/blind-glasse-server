...
class StreamManager:
    ...
    def _url_is_recursive_public_stream(url: str) -> bool:
        ...
        try:
            ...
            public_hosts = {
                str(x).lower()
                for x in (getattr(config, "PUBLIC_SITE_HOSTS", None) or [])
            }
            if h in public_hosts and port in (80, 443):
                return True
    ...
    @staticmethod
    def _lan_pull_enabled() -> bool:
        if bool(getattr(config, "STREAM_ALLOW_LAN_PULL", False)):
            return True
        return bool((getattr(config, "ESP32_STREAM_URL", "") or "").strip())

    def set_esp32_ip(self, ip: str) -> None:
        """設定 ESP32 IP（由 UDP 發現或首次請求取得），並啟動拉流（若尚未啟動）。"""
        if not self._lan_pull_enabled():
            return
        with self._lock:
    ...
    def _start_puller(self, esp32_ip: str) -> None:
        ...
        def run() -> None:
            ...
            log_throttled(f"stream-pull-start:{url}", f"[Stream] Pulling MJPEG from {url!r}")
            ...
            while not self._puller_stop.is_set():
                ...
                try:
                    ...
                    if not self._puller_stop.is_set():
                        now = time.time()
                        iv = float(getattr(config, "SERVER_QUIET_LOG_SEC", 300.0))
                        if now - self._last_pull_err_log_ts >= iv:
                            log_throttled(f"stream-pull-err:{url}", f"[Stream] Pull error: {e}")
        ...
        log_throttled(f"stream-puller-thread:{esp32_ip}", f"[Stream] Puller started for {esp32_ip}")
