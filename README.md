...
- **家裡／實驗室長開一台機器**：用 **Cloudflare Tunnel** 把本機 `localhost:4000` 接到 `https://www.blind-glasses.org`，眼鏡設 **`CLOUD_MODE=1`** 即可從外面連線。詳見專案內 **[Cloudflare Tunnel 部署指南](../docs/cloudflare-tunnel-setup.md)**；Windows 可雙擊專案根目錄 **`start_cloud.bat`**（需先在 `server/.env` 設定 `CLOUDFLARE_TUNNEL_TOKEN`）。
...
- **韌體 Header（裝置端）**：`CLOUD_MODE=1` 時可在 `firmware/include/config.h`（整包工作區；若單獨 clone 韌體倉庫則為根目錄 `include/config.h`）設定 `DEVICE_PUBLIC_STREAM_URL`；眼鏡每次 POST `/api/imu` 會帶 `X-Device-Stream-Url`，伺服器優先用此 URL 拉流（適用 [監控頁](https://www.blind-glasses.org/monitor) 經 CDN、無法用「請求來源 IP:81」的情境）。若同時設了 `ESP32_STREAM_URL`，仍以裝置 Header 為優先。
...
Monitor overlay can also load YOLO `.pt` weights directly through Ultralytics.
The default imported model is:

```env
YOLOE_MODEL_PATH=models/yolo.pt
TRAFFIC_LIGHT_YOLO_MODEL_PATH=models/yolo.pt
VISION_MODEL_PATHS=models/yolo.pt
VISION_TARGET_CLASSES=
VISION_DETECT_CONF_THRES=0.35
VISION_DETECT_IOU_THRES=0.45
BLIND_TILE_MODEL_PATH=models/yolo.pt
BLIND_TILE_TARGET_CLASSES=road_crossing,blind_path
```

`VISION_MODEL_PATHS` accepts comma-separated `.pt` paths. Leave
`VISION_TARGET_CLASSES` empty to draw every detected class, or set values such
as `traffic_light,road_crossing,blind_path` to filter the overlay.
...
```bash
cd server
python benchmark_latency.py --host 127.0.0.1 --port 4000 --runs 30
```
...
1. **先開伺服器**（在 `server` 目錄）：
   - 雙擊 `run.bat`，或：`uvicorn main:app --host 0.0.0.0 --port 4000`
   - 看到 `Uvicorn running on http://0.0.0.0:4000` 即表示就緒。
...
| 步驟 | 做法 | 預期結果 |
|------|------|----------|
| 1. 連線 | 瀏覽器開 `http://<你電腦的IP>:4000/health`（與眼鏡同 WiFi 的手機或電腦） | 看到 `{"status":"ok","server_ip":"..."}` |
| 2. 眼鏡找伺服器 | 看 Serial Monitor 或等眼鏡開機完成 | 應有「找到伺服器」或類似 log，之後才會送 API |
| 3. 送 GPS（導航起點） | 到**戶外或窗邊**讓 GPS 定位；或先用電腦模擬：`curl -X POST http://<電腦IP>:4000/api/gps -H "Content-Type: application/json" -d "{\"lat\":25.033,\"lng\":121.565}"` | 伺服器會記住這筆位置，60 秒內說「導航到家」會用這當起點 |
| 4. 觸發導航模式 | `curl -X POST "http://<電腦IP>:4000/api/gemini?mode=general"`（或由前端/測試工具呼叫） | 伺服器應產生新的 TTS，`/audio/latest` 可下載最新音檔 |
...
- **確認 TTS 有產生**：用瀏覽器開 `http://<電腦IP>:4000/audio/latest`，若有檔案會下載 mp3；若 404 表示尚未有語音佇列產出。
