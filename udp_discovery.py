...
def _get_local_ip(peer_host: Optional[str] = None) -> str:
    override = (getattr(config, "SERVER_IP", "") or "").strip()
    if override:
        return override
    target = peer_host or "8.8.8.8"
    try:
...
def run_udp_listener(on_esp32_seen: Optional[Callable[[str], None]] = None) -> None:
    ...
    my_ip = _get_local_ip()
    print(f"[UDP] Listening on port {config.UDP_PORT}, default reply IP: {my_ip}")
    ...
    while True:
        try:
            ...
            if msg == config.UDP_DISCOVERY_MSG:
                reply_ip = _get_local_ip(addr[0])
                response = f"{config.UDP_RESPONSE_PREFIX}{reply_ip}".encode()
                sock.sendto(response, addr)
                print(f"[UDP] Replied to {addr[0]} -> SERVER_IP: {reply_ip}")
                if on_esp32_seen:
