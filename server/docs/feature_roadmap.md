...
| `yolo-seg.pt` | YOLO 分割模型（備用/輕量版） | 可作為 fallback |
| `trafficlight.pt` | 紅綠燈專用偵測模型 | **可接入** `traffic_crossing.py` |
| `tactile_paving_detector.pt` | 導盲磚/斑馬線偵測 | **已接入** `vision_controller.py` → `BlindTileDetector` |
